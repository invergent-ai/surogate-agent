  ...
  template: `
    <div class="flex flex-col gap-3">
      ...
      <button
        (click)="acknowledge()"
        [disabled]="submitting()"
        class="w-full py-2 px-3 rounded-lg text-sm font-medium bg-blue-500 hover:bg-blue-600 text-white transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
      >
...
export class ReportTaskComponent implements OnChanges, OnDestroy {
  ...
  acknowledge(): void {
    ...
    this.taskSvc.respond(this.task.id, { acknowledged: true, feedback: this.feedback }).subscribe({
